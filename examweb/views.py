...
def openplat(request):
    username=request.session.get('username',None)
    name=request.session.get('name',None)
    is_developer = check_permission(username,'developer')
    if request.method=="POST":
        print(request.POST.getlist('lang[]',None))
        return_param = {}
        return_param['status'] = 200
        return HttpResponse(json.dumps(return_param))
    return render(request,'open.html',locals())
...
def register(request):
    ...
    is_developer = check_permission(username,'developer')
    if request.method == 'POST':
        reg_form=DevForm(request.POST)
        reg_form.fields['username'].initial = username
        reg_form.fields['username'].widget.attrs['readonly'] = True
        reg_form.fields['name'].initial = name
        reg_form.fields['name'].widget.attrs['readonly'] = True
        if reg_form.is_valid():
            tusername = reg_form.cleaned_data['username']
            tname = reg_form.cleaned_data['name']
            contact = reg_form.cleaned_data['contact']
            usefor = reg_form.cleaned_data['usefor']
            if tusername != username or tname != name:
                message = "系统审核失败:请勿更改您的姓名和准考证号"
